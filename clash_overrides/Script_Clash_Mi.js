// iPad 、iPhone 的 Clash Mi 专用 第一个分组必须是节点选择
function main(config, profileName) {
    ...
    // 将所有已有的“走代理”规则统一改到“节点选择”。
    // DIRECT/REJECT/PASS 这类明确直连、拦截或透传的规则保持不变。
    function forceRulePolicyToNodeSelect(rule) {
        if (typeof rule !== "string") {
            return rule;
        }
        ...
        for (let i = parts.length - 1; i >= 1; i -= 1) {
            ...
            // 已经是“节点选择”的规则不重复改写，保留原格式。
            if (value === aiGroupName) {
                return rule;
            }
            ...
            const leading = parts[i].match(/^\s*/)[0];
            const trailing = parts[i].match(/\s*$/)[0];
            parts[i] = `${leading}${aiGroupName}${trailing}`;
            return parts.join(",");
        }
    ...
    const newRules = forceProxyDomains.map(
        (domain) => `DOMAIN-SUFFIX,${domain},${aiGroupName}`,
    );
    ...
    // 第一步：把已知 AI/自动策略名改到“节点选择”。
    // 第二步：把剩余非 DIRECT/REJECT/PASS 的代理规则也统一改到“节点选择”。
    const fallbackRulePolicyNames = new Set(
        unique([
            ...
            ...aiAutoGroupCandidates,
            ...autoGroupNames,
        ]).filter((name) => name !== aiGroupName),
    );
    const rewrittenOldRules = cleanedOldRules
        .map((rule) =>
            replaceRulePolicy(rule, fallbackRulePolicyNames, aiGroupName),
        )
        .map(forceRulePolicyToNodeSelect);

    // 规则顺序很关键：显式直连优先，其次是强制走“节点选择”的域名，
    // 最后才放入已经统一改写到“节点选择”的原有规则。
    config.rules = directIpRules.concat(directRules, newRules, rewrittenOldRules);

    // Clash Mi 首页通常展示 proxy-groups 中排在最前面的策略组。
    // 将“节点选择”移动到第一位，避免首页一直显示 AI自动兜底当前选中的新加坡节点。
    const nodeSelectGroupIndex = proxyGroups.findIndex(
        (group) => group && group.name === aiGroupName,
    );
    if (nodeSelectGroupIndex > 0) {
        const [nodeSelectGroup] = proxyGroups.splice(nodeSelectGroupIndex, 1);
        proxyGroups.unshift(nodeSelectGroup);
    }
