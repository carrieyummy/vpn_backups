...
- 本项目用于备份我的 VPN 配置，不要暴露个人信息，如 `10.100.10.xx`、真实订阅 URL、代理地址、面板密钥、provider 节点文件内容。如果发现个人信息，先停止展示具体值，询问我怎么处理，给我处理方案供选择。
...
- `Script.local.js` 是 `Script.js` 的本地版本，不要用 `git add -f` 强制提交。`Script.local.js` 包含了个人自定义域名（从 start 到 end）。同步规则：`Script.js` -> `Script.local.js`，个人自定义域名不要出现在 `Script.js` 里。
